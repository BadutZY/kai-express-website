            ...
            <div class="k-card-body" style="text-align:center;padding:28px 20px;">
                <div class="photo-wrap">
                    @if($user->photo_profile)
                        <img id="profilePhotoPreview" src="{{ Storage::disk('public')->url($user->photo_profile) }}?v={{ $user->updated_at->timestamp }}" alt="foto profil">
                    @else
                        <div class="photo-init" id="profilePhotoInit">{{ strtoupper(substr($user->name,0,2)) }}</div>
                ...
                <div style="font-family:var(--display);font-size:17px;font-weight:900;margin-bottom:4px;">{{ $user->name }}</div>
                <div style="font-size:12px;color:var(--muted);margin-bottom:16px;">{{ $user->email }}</div>
                @if($user->photo_profile)
                <form method="POST" action="{{ route('user.profil.remove-photo') }}">
                    @csrf
    ...
    <div class="col-lg-8">
        ...
        <div class="k-card mb-3">
            <div class="k-card-body">
                ...
                {{-- Form ini hanya untuk upload foto --}}
                <form method="POST" action="{{ route('user.profil.update') }}" enctype="multipart/form-data" id="profileForm">
                    @csrf
                    <input type="file" id="photoFileInput" name="photo_profile" accept="image/jpeg,image/png,image/webp" style="display:none;">
                    <div class="form-group"><label class="form-label">Nama Lengkap</label><input type="text" name="name" value="{{ $user->name }}" class="form-input" required></div>
                    <div class="form-group"><label class="form-label">Email</label><input type="email" value="{{ $user->email }}" class="form-input" disabled style="opacity:.5;cursor:not-allowed;"><div style="font-size:11px;color:var(--muted);margin-top:5px;">Email tidak dapat diubah</div></div>
                    <div class="form-group"><label class="form-label">NIK</label><input type="text" name="nik" value="{{ $user->nik }}" class="form-input" maxlength="16" placeholder="3201XXXXXXXXXXXX"></div>
                    <div class="form-group"><label class="form-label">No. HP</label><input type="text" name="no_hp" value="{{ $user->no_hp }}" class="form-input"></div>
...
<form method="POST" action="{{ route('user.profil.update') }}" enctype="multipart/form-data" id="photoUploadForm" style="display:none;">
    @csrf
