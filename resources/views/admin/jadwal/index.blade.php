...
<div class="page-header d-flex align-items-center justify-content-between flex-wrap gap-3">
    ...
    <a href="{{ route('admin.jadwal.create') }}" class="btn-primary"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M12 4.5v15m7.5-7.5h-15"/></svg>Tambah Jadwal</a>
</div>
<div class="k-card mb-4 fade-up"><div class="k-card-body" style="padding:14px 18px;">
    <form method="GET" action="{{ route('admin.jadwal.index') }}">
        <div class="row g-2 align-items-center">
            <div class="col-md-5"><div class="search-wrap"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M21 21l-5.197-5.197m0 0A7.5 7.5 0 105.196 5.196a7.5 7.5 0 0010.607 10.607z"/></svg><input type="text" name="search" value="{{ $search }}" placeholder="Cari stasiun..." class="form-input"></div></div>
            <div class="col-md-3"><input type="text" name="tujuan" value="{{ $tujuan }}" placeholder="Filter tujuan (cth: Bandung)" class="form-input"></div>
            <div class="col-md-2"><button type="submit" class="btn-primary w-100 justify-content-center">Filter</button></div>
            <div class="col-md-2"><a href="{{ route('admin.jadwal.index') }}" class="btn-secondary w-100 justify-content-center">Reset</a></div>
        </div>
    </form>
</div></div>
<div class="k-card fade-up d1">
    ...
    <span style="font-size:12px;color:var(--text-muted);">{{ $jadwal->total() }} jadwal</span></div>
    <div style="overflow-x:auto;"><table class="k-table">
        ...
        <tbody>
            ...
            <tr>
                <td style="color:var(--text-muted);font-size:12px;">{{ $jadwal->firstItem()+$i }}</td>
                <td><div style="font-weight:600;font-size:13px;">{{ $j->kereta->nama_kereta }}</div><span class="badge-kelas {{ strtolower($j->kereta->kelas) }}">{{ $j->kereta->kelas }}</span></td>
                ...
                <td><div style="display:flex;gap:5px;">
                    ...
                    <form id="del-j-{{ $j->id_jadwal }}" action="{{ route('admin.jadwal.destroy',$j->id_jadwal) }}" method="POST" style="display:inline;">@csrf @method('DELETE')<button type="button" class="btn-icon delete" onclick="confirmDelete('del-j-{{ $j->id_jadwal }}')"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M14.74 9l-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 01-2.244 2.077H8.084a2.25 2.25 0 01-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 00-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 013.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 00-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 00-7.5 0"/></svg></button></form>
                </div></td>
            </tr>
            @empty
    ...
    @if($jadwal->hasPages())<div class="k-card-body" style="padding:14px 18px;">{{ $jadwal->appends(['search'=>$search,'tujuan'=>$tujuan])->links() }}</div>@endif
</div>
@endsection
