    ...
    <div class="col-lg-7 fade-up d1">
        <div class="k-card h-100">
            <div class="k-card-header">
                ...
                <a href="{{ route('admin.pemesanan.index') }}" class="btn-secondary" style="padding:6px 12px;font-size:12px;">Lihat Semua</a>
            </div>
                        ...
                        <tr>
                            <td>
                                <div style="display:flex;align-items:center;gap:10px;">
                                    <div style="width:34px;height:34px;border-radius:9px;background:var(--bg-surface3);border:1px solid var(--border);display:flex;align-items:center;justify-content:center;font-size:12px;font-weight:700;color:var(--text-secondary);flex-shrink:0;">{{ strtoupper(substr($p->penumpang->nama_penumpang,0,2)) }}</div>
                                    <div>
                                        <div style="font-weight:600;font-size:13px;">{{ $p->penumpang->nama_penumpang }}</div>
                                        <div style="font-size:11px;color:var(--text-muted);">{{ $p->user->name ?? 'Guest' }}</div>
                                    </div>
                                </div>
                            </td>
                            <td>
                            ...
                            <td>
                                @php $sc = ['confirmed'=>'low','pending'=>'','cancelled'=>'high']; @endphp
                                <span class="badge-count {{ $sc[$p->status] ?? '' }}" style="{{ $p->status=='pending' ? 'background:var(--gold-soft);color:var(--gold);border:1px solid rgba(200,149,42,.25);' : '' }}">{{ $p->status }}</span>
                            </td>
                        </tr>
                        @empty
