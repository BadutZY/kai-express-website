...
<div class="k-card mb-4 fade-up"><div class="k-card-body" style="padding:14px 18px;">
    <form method="GET" action="{{ route('admin.users.index') }}">
        <div class="row g-2 align-items-center">
            <div class="col-md-8"><div class="search-wrap"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M21 21l-5.197-5.197m0 0A7.5 7.5 0 105.196 5.196a7.5 7.5 0 0010.607 10.607z"/></svg><input type="text" name="search" value="{{ $search }}" placeholder="Cari nama atau email..." class="form-input"></div></div>
            <div class="col-md-2"><button type="submit" class="btn-primary w-100 justify-content-center">Cari</button></div>
            <div class="col-md-2"><a href="{{ route('admin.users.index') }}" class="btn-secondary w-100 justify-content-center">Reset</a></div>
        </div>
    </form>
</div></div>
<div class="k-card fade-up d1">
    ...
    <span style="font-size:12px;color:var(--text-muted);">{{ $users->total() }} pengguna</span></div>
            ...
            <tr>
                ...
                <td>
                    <div style="display:flex;align-items:center;gap:10px;">
                        @if($u->photo_profile)
                            <img src="{{ Storage::disk('public')->url($u->photo_profile) }}" alt="{{ $u->name }}"
                                 style="width:36px;height:36px;border-radius:9px;object-fit:cover;border:1px solid rgba(91,63,166,.25);flex-shrink:0;">
                        @else
                            <div style="width:36px;height:36px;border-radius:9px;background:var(--purple-soft);border:1px solid rgba(91,63,166,.25);display:flex;align-items:center;justify-content:center;font-size:12px;font-weight:700;color:#a78bfa;flex-shrink:0;">{{ strtoupper(substr($u->name,0,2)) }}</div>
                        @endif
                        <div>
                ...
                <td><span class="mono" style="background:var(--bg-surface2);padding:3px 8px;border-radius:5px;border:1px solid var(--border);font-size:11px;">{{ $u->nik ?? '-' }}</span></td>
                <td style="color:var(--text-secondary);font-size:13px;">{{ $u->no_hp ?? '-' }}</td>
                <td><span class="badge-count {{ $u->pemesanan_count>0?'low':'zero' }}">{{ $u->pemesanan_count }} order</span></td>
                <td><a href="{{ route('admin.users.show',$u->id) }}" class="btn-icon view"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M2.036 12.322a1.012 1.012 0 010-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.964-7.178z"/><path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/></svg></a></td>
            </tr>
            @empty
    ...
    @if($users->hasPages())<div class="k-card-body" style="padding:14px 18px;">{{ $users->appends(['search'=>$search])->links() }}</div>@endif
</div>
@endsection
