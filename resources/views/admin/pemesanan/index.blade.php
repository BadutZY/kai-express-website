...
<div class="k-card mb-4 fade-up"><div class="k-card-body" style="padding:14px 18px;">
    <form method="GET" action="{{ route('admin.pemesanan.index') }}">
        <div class="row g-2 align-items-center">
            <div class="col-md-5"><div class="search-wrap"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M21 21l-5.197-5.197m0 0A7.5 7.5 0 105.196 5.196a7.5 7.5 0 0010.607 10.607z"/></svg><input type="text" name="search" value="{{ $search }}" placeholder="Cari nama penumpang..." class="form-input"></div></div>
            <div class="col-md-3"><select name="filter_tiket" class="form-input"><option value="">Semua Pemesanan</option><option value="lebih2" {{ $filterTiket=='lebih2'?'selected':'' }}>Tiket lebih dari 2</option></select></div>
            <div class="col-md-2"><button type="submit" class="btn-primary w-100 justify-content-center">Filter</button></div>
            <div class="col-md-2"><a href="{{ route('admin.pemesanan.index') }}" class="btn-secondary w-100 justify-content-center">Reset</a></div>
        </div>
    </form>
</div></div>
<div class="k-card fade-up d1">
    ...
    <span style="font-size:12px;color:var(--text-muted);">{{ $pemesanan->total() }} transaksi</span></div>
                ...
                <td>
                    <div style="display:flex;align-items:center;gap:10px;">
                        ...
                        <div>
                            ...
                            <div style="font-size:11px;color:var(--text-muted);">{{ $p->user->name ?? '-' }}</div>
                        </div>
                    </div>
                </td>
                <td><div style="font-weight:600;font-size:13px;">{{ $p->jadwal->kereta->nama_kereta }}</div><div class="route-wrap" style="margin-top:2px;"><span style="font-size:11px;color:var(--text-muted);">{{ $p->jadwal->stasiun_asal }}</span><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2" style="width:11px;height:11px;"><path stroke-linecap="round" stroke-linejoin="round" d="M13.5 4.5L21 12m0 0l-7.5 7.5M21 12H3"/></svg><span style="font-size:11px;color:var(--text-muted);">{{ $p->jadwal->stasiun_tujuan }}</span></div></td>
    ...
    @if($pemesanan->hasPages())<div class="k-card-body" style="padding:14px 18px;">{{ $pemesanan->appends(['search'=>$search,'filter_tiket'=>$filterTiket])->links() }}</div>@endif
</div>
@endsection
