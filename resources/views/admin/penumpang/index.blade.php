...
<div class="k-card mb-4 fade-up"><div class="k-card-body" style="padding:14px 18px;">
    <form method="GET" action="{{ route('admin.penumpang.index') }}">
        <div class="row g-2 align-items-center">
            <div class="col-md-8"><div class="search-wrap"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M21 21l-5.197-5.197m0 0A7.5 7.5 0 105.196 5.196a7.5 7.5 0 0010.607 10.607z"/></svg><input type="text" name="search" value="{{ $search }}" placeholder="Cari nama, NIK, atau HP..." class="form-input"></div></div>
            <div class="col-md-2"><button type="submit" class="btn-primary w-100 justify-content-center">Cari</button></div>
            <div class="col-md-2"><a href="{{ route('admin.penumpang.index') }}" class="btn-secondary w-100 justify-content-center">Reset</a></div>
        </div>
    </form>
</div></div>
<div class="k-card fade-up d1">
    ...
    <span style="font-size:12px;color:var(--text-muted);">{{ $penumpang->total() }} data</span></div>
            ...
            <tr>
                ...
                <td>
                    <div style="display:flex;align-items:center;gap:10px;">
                        @if($userPhoto)
                            <img src="{{ Storage::disk('public')->url($userPhoto) }}" alt="{{ $p->nama_penumpang }}"
                                 style="width:36px;height:36px;border-radius:9px;object-fit:cover;border:1px solid var(--border);flex-shrink:0;">
                        @else
                            <div style="width:36px;height:36px;border-radius:9px;background:var(--bg-surface3);border:1px solid var(--border);display:flex;align-items:center;justify-content:center;font-size:12px;font-weight:700;color:var(--text-secondary);flex-shrink:0;">{{ $initials }}</div>
                        @endif
                        <div>
    ...
    @if($penumpang->hasPages())<div class="k-card-body" style="padding:14px 18px;">{{ $penumpang->appends(['search'=>$search])->links() }}</div>@endif
</div>
@endsection
