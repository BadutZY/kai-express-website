...
<body>
    <div class="auth-wrap">
        ...
        <div class="auth-card">
            <!-- Tabs -->
            <div class="auth-tabs">
                <div class="auth-tab {{ !request('tab') || request('tab')=='login' ? 'active' : '' }}" onclick="switchTab('login')">Masuk</div>
                <div class="auth-tab {{ request('tab')=='forgot' ? 'active' : '' }}" onclick="switchTab('forgot')">Lupa Password</div>
            </div>

            <!-- LOGIN PANEL -->
            <div class="tab-panel {{ !request('tab') || request('tab')=='login' ? 'active' : '' }}" id="tab-login">
                @if($errors->any() && !request('tab'))
            ...
            <div class="tab-panel {{ request('tab')=='forgot' ? 'active' : '' }}" id="tab-forgot">
                @if($errors->any() && request('tab')=='forgot')
                <div class="err">{{ $errors->first() }}</div>
                @endif
                @if(session('forgot_success'))
                <div class="ok">{{ session('forgot_success') }}</div>
                @endif
                ...
                <form method="POST" action="{{ route('forgot.verify') }}">
                    ...
                    <div class="form-group">
                        ...
                        <input type="email" name="email" value="{{ old('forgot_email') }}" class="form-input" placeholder="nama@email.com" required>
                    </div>
                ...
                <div style="margin-top:20px;padding-top:20px;border-top:1px solid var(--border);">
                    <p style="font-size:13px;color:var(--muted);margin-bottom:16px;">Akun ditemukan. Silakan masukkan password baru untuk <strong style="color:var(--text);">{{ session('reset_email') }}</strong></p>
